...
let Fetcher = require(dirs.exchanges + 'abucoins');
...
Fetcher.prototype.getTrades = function(sinceTid, callback) {
  ...
  let handler = (cb) => this.abucoins_public.getProductTrades(currencyPair, { after: sinceTid, limit: BATCH_SIZE }, this.handleResponse('getTrades', cb));
...
Fetcher.prototype.findFirstTrade = function(sinceTs, callback) {
  ...
  let process = function(err, data) {
    ...
    setTimeout(() => {
      let handler = (cb) => this.abucoins_public.getProductTrades(currencyPair, { after: nextScanId, limit: 1 }, this.handleResponse('getTrades', cb));
      util.retryCustom(retryForever, _.bind(handler, this), _.bind(process, this));
    }, QUERY_DELAY);
  }
  ...
  let handler = (cb) => this.abucoins_public.getProductTrades(currencyPair, { limit: 1 }, this.handleResponse('getTrades', cb));
  util.retryCustom(retryForever, _.bind(handler, this), _.bind(process, this));
}
...
let handleFetch = (err, trades) => {
  if (err) {
    log.error(`There was an error importing from Abucoins ${err}`);
    fetcher.emit('done');
